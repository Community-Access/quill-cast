# Builds the QUILL Cast installer from the PyInstaller one-file build.
#
# Usage:
...
if (-not (Test-Path (Join-Path $repoRoot "dist\QUILLCast.exe"))) {
    throw "dist\QUILLCast.exe not found; run scripts\build_exe.ps1 first."
}
...
if (-not $FfmpegDir -or -not (Test-Path (Join-Path $FfmpegDir "ffmpeg.exe"))) {
    throw "ffmpeg.exe not found. Pass -FfmpegDir; audio processing must ship bundled."
}
...
& $Iscc (Join-Path $repoRoot "installer\quill-cast.iss") `
